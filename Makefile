...
download-sdk:
	@if [ ! -f "$(SDK_ZIP_FILEPATH)" ]; then \
		SDK_DOWNLOAD_URL="https://partner.steamgames.com/downloads/$(SDK_NAME)"; \
		echo "Downloading SDK from: $$SDK_DOWNLOAD_URL"; \
		curl -LO --output-dir "build/" $$SDK_DOWNLOAD_URL; \
	else \
...
sdk: download-sdk
	...
	unzip -u -j -d "$(LIB_PATH)" "$(SDK_ZIP_FILEPATH)" "sdk/redistributable_bin/$(PLATFORM)/libsteam_api.so"
	# extract library's headers
	unzip -u -j -d "$(SDK_PATH)" "$(SDK_ZIP_FILEPATH)" "sdk/public/steam/*.h"
